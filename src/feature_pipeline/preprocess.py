"""
Preprocessing Script for Housing Regression

- Reads train/eval/holdout CSVs from data/raw/ .
- Cleans and normamlizes city names.
- Maps cities to metros dataset and merges lat/lng.
- Drops extreme outliers.
...
def normalize_metro_name(s: str):
    """Normalize metro label and drop state suffix after comma."""
    s = normalize_city(s)
    if pd.isna(s):
        return s
...
def clean_and_merge(df: pd.DataFrame, metros_path: str | None = "data/raw/usmetros.csv") -> pd.DataFrame:
    ...
    df["city_key"] = df["city_full"].apply(normalize_metro_name)
    ...
    metros = (
        metros.assign(
            city_key=metros["metro_full"].apply(normalize_metro_name),
            population_num=pd.to_numeric(metros.get("population"), errors="coerce"),
        )
        .sort_values("population_num", ascending=False)
        .drop_duplicates(subset=["city_key"], keep="first")
    )
    df = df.merge(metros[["city_key", "lat", "lng"]], how="left", on="city_key")
    df.drop(columns=["city_key"], inplace=True, errors="ignore")
