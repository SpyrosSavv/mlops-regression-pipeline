"""
Feature engineering: date parts, frequency encoding, target encoding, drop data leakage columns.

- Reads cleaned train/eval csv
- Applies feature engineering
- Saves feature engineered csv
...
# Create a frequency encoding (how often a value appers).
...
def target_encode(train: pd.DataFrame, eval: pd.DataFrame, col: str, target: str):
    ...
    For city_full → city_encoded (keeps schema aligned with notebooks).
    """
...
# Read cleaned csvs, apply feature engineering and save engineered data and encoders
